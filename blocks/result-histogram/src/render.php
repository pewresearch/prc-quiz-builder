...
$same_as = array_values(array_filter($data, function ($item) use($score) {
	return ($item->x === $score);
}));

$better_than = array_values(array_filter($data, function ($item) use($score) {
	return ($item->x < $score);
}));

$below = array_values(array_filter($data, function ($item) use($score) {
	return ($item->x > $score);
}));
...
$better_than_sum = array_reduce($better_than, function($carry, $item) {
	$carry += $item->y;
	return $carry;
});
...
$below_sum = array_reduce($below, function($carry, $item) {
	$carry += $item->y;
	return $carry;
});
