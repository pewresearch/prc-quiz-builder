...
import { PanelBody, ToggleControl, TextControl, CardDivider, Button, Notice as WPComNotice, Tooltip, ToolbarGroup } from '@wordpress/components';
...
const ToolbarButtonText = styled.span`
	color: ${yellowColor};
`;
	...
	return (
		<BlockControls>
			<ToolbarGroup
				controls={[
					{
						children: <ToolbarButtonText>{selectLabel}</ToolbarButtonText>,
						title: selectLabel,
						onClick: () => {
...
export function ConditionalDot({attributes}) {
	...
	return (
		<Tooltip text={__(`This will only display if the user answers: "${conditionalAnswer}".`, 'prc-quiz')}>
			<Dot onClick={() => {
				selectBlock(conditionalAnswerClientId);
			}}>&#8226;</Dot>
		</Tooltip>
...
export function ConditionalPanel({attributes, setAttributes, children, blockType = 'question'}) {
		...
		<PanelBody title={__('Conditional Display Settings')}>
			<div>
				{conditionalDisplay && conditionalAnswerUuid && (
					<Fragment>
						<ConditionalNotice attributes={attributes} blockType={blockType} />
						<CardDivider/>
					</Fragment>
				)}
				{children}
				<ToggleControl
					...
					onChange={() => {
						setAttributes({ conditionalDisplay: !conditionalDisplay });
					}}
					help={__(
