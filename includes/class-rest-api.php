...
class Rest_API {
	...
	public function verify_nonce($quiz_id, $key, $nonce) {
		if ( wp_verify_nonce( $nonce, $key . $quiz_id ) ) {
			...
			return new \WP_Error( 'invalid_nonce', 'Unauthorized access. ERROR CODE: 403', array( 'status' => 403 ) );
		}
	}
	...
	public function create_group( $group_name, $owner_id, $quiz_id = false, $typology_groups = [] ) {
		if ( false === $quiz_id ) {
			return new WP_Error(
				'invalid_quiz_id',
