...
- Legacy community-groups MySQL layer (BerlinDB `Community_Groups_Table` / `Community_Groups_Query` and `includes/legacy-groups/`). Groups are Firebase-only; lazy migration from MySQL has been removed.
- Composer dependency `berlindb/core`. The legacy `prc_quiz_community_groups` table may remain in the database for manual DBA cleanup.
